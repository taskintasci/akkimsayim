          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '7pt' }}>
            <thead>
              <tr style={{ background: '#1e293b', color: 'white' }}>
                ...
                <th style={thStyle({ textAlign: 'left',   width: 85 })}>ADRES</th>
                <th style={thStyle({ textAlign: 'left',   width: 85 })}>STOK KODU</th>
                <th style={thStyle({ textAlign: 'left',   width: 220 })}>STOK ADI</th>
                <th style={thStyle({ textAlign: 'center', width: 90 })}>BEYANNAME</th>
                <th style={thStyle({ textAlign: 'left',   width: 100 })}>PALET BARKODU</th>
                <th style={thStyle({ textAlign: 'right',  width: 55 })}>PALET ADETİ</th>
                <th style={thStyle({ textAlign: 'right',  width: 60 })}>TOPLAM STOK</th>
                <th style={thStyle({ textAlign: 'right',  width: 60 })}>REZERVE ADET</th>
                <th style={thStyle({ textAlign: 'center', width: 65, background: '#1d4ed8' })}>SAYIM MİKTARI</th>
                <th style={thStyle({ textAlign: 'center', width: 55 })}>BİRİM ADI</th>
              ...
              {pageItems.map((item, i) => {
                ...
                return (
                  <tr key={item.id || i} style={{ background: rowBg }}>
                    ...
                    <td style={{ padding: '2px 4px', border: '1px solid #e2e8f0', fontFamily: 'monospace', fontSize: '6pt', color: '#64748b' }}>{item.paletBarkodu}</td>
                    <td style={{ padding: '2px 4px', textAlign: 'right', border: '1px solid #e2e8f0', fontFamily: 'monospace' }}>{item.paletAdeti}</td>
                    <td style={{ padding: '2px 4px', textAlign: 'right', border: '1px solid #e2e8f0', fontFamily: 'monospace' }}>{item.adet1}</td>
                    <td style={{ padding: '2px 4px', textAlign: 'right', border: '1px solid #e2e8f0', fontFamily: 'monospace' }}>{item.rezerveAdet}</td>
                    <td style={{
