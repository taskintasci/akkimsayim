...
export default function KullaniciYonetim() {
  ...
  return (
    <div className="max-w-4xl">
      <h1 className="text-2xl font-bold text-slate-800 mb-1">Kullanıcı Yönetimi</h1>
      <p className="text-sm text-slate-500 mb-6">Kullanıcı hesapları oluşturun ve rollerini yönetin.</p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
        ...
        <form onSubmit={handleCreate} className="grid grid-cols-2 gap-4">
          ...
          {error && (
            <div className="col-span-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-red-700 text-sm">{error}</div>
          )}
          {okMsg && (
            <div className="col-span-2 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2 text-emerald-700 text-sm">{okMsg}</div>
          )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                    ...
                    <td className="px-5 py-3 text-right">
                      {isSelf ? (
                        ...
                        <span className="inline-flex items-center gap-1.5">
                          <button
                            onClick={async () => { await deleteUserDoc(u.uid); setConfirmId(null) }}
                            className="px-2 py-1 bg-red-500 hover:bg-red-600 text-white text-[11px] font-semibold rounded-md"
                          >Sil</button>
                          <button
                            onClick={() => setConfirmId(null)}
                            className="px-2 py-1 text-slate-400 hover:text-slate-600 text-[11px] rounded-md"
                          >İptal</button>
                        </span>
                      ) : (
                        <button
                          onClick={() => setConfirmId(u.uid)}
        ...
        <p className="px-5 py-3 text-[11px] text-slate-400 border-t border-slate-50">
          Not: Silme işlemi kullanıcının erişim profilini kaldırır. Firebase Authentication hesabı,
          gerekirse Firebase Console üzerinden ayrıca silinmelidir.
        </p>
      </div>
    </div>
  )
